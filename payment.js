...
function checkoutErrorMessage(error) {
  switch (error?.code) {
    case "auth/existing-account-password-mismatch":
      return "This email already has a portal account. Use the same password used earlier, or contact support to reset the old incomplete account before checkout.";
    case "auth/invalid-credential":
    case "auth/wrong-password":
      return "This email already exists, but the password entered here does not match it. Use the old portal password or contact support to reset the account.";
    case "auth/too-many-requests":
      return "Too many attempts for this email. Please wait a few minutes, then try again or contact support.";
    case "auth/weak-password":
      return "Create a stronger portal password with at least 8 characters.";
    case "auth/invalid-email":
      return "Please enter a valid email address.";
    default:
      if (error?.message === "Failed to fetch") {
        return "Payment server is live, but this website is not allowed to call it yet. Check Cloudflare ALLOWED_ORIGINS includes https://resettraders.com, then redeploy the Worker.";
      }
      return error?.message || "Checkout could not start. Please try again.";
  }
}
