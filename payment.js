...
function toggleCheckoutBusy(isBusy) {
  ...
  button.textContent = isBusy ? "Preparing secure checkout..." : "🔒 Continue to Payment →";
}
...
function setSuccessStatus(message, state = "pending") {
  ...
  if (icon) icon.textContent = state === "verified" ? "✓" : "⌛";
...
export async function proceedToRazorpay() {
  ...
  try {
    ...
    const message = error?.message === "Failed to fetch"
      ? "Payment server is not connected yet. Deploy the Razorpay worker first, then try checkout again."
